...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, x_shortcut, t=None):
        ...
        # Pad x to match x_shortcut size
        if x.size(2) != x_shortcut.size(2) or x.size(3) != x_shortcut.size(3):
            diff_h = x_shortcut.size(2) - x.size(2)
            diff_w = x_shortcut.size(3) - x.size(3)
            x = nn.functional.pad(
                x,
